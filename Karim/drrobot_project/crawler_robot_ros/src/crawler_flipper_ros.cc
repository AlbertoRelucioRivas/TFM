...
*/
  ...
  ros::Subscriber flipper_info_subscriber_;
  ros::Publisher  odom_publisher_;

  std::string robotnamespace_;
  std::string topicname_cmdvel_;
  std::string topicname_cmdflp_;
  std::string topicname_flpinfo_;
  ...
  {
    Target_FLP_FR = Target_FLP_FL = Target_FLP_RR = Target_FLP_RL = 0.;// M_PI / 4;
    wheel_separation_ = 1;
    wheel_diameter_ = 0.4;
    torque = 5.0;
...
//    alive_ = false;
  ...
  /////////////////////////////////////////////////
  void velCallback(const geometry_msgs::TwistStamped::ConstPtr& cmd_msg)
  {
    boost::mutex::scoped_lock scoped_lock(lock_);
    double vlin = cmd_msg->twist.linear.y / (wheel_diameter_/2);
    double vrot = cmd_msg->twist.angular.z 
                                    * wheel_separation_ / (wheel_diameter_/2);
...
*/
  ...
  /////////////////////////////////////////////////
  void flipperCallback(const geometry_msgs::TwistStamped::ConstPtr& cmd_msg)
  {
    boost::mutex::scoped_lock scoped_lock(lock_);
    Target_FLP_FR = cmd_msg->twist.linear.x;
    Target_FLP_FL = cmd_msg->twist.linear.x;
    Target_FLP_RR = cmd_msg->twist.angular.y;
    Target_FLP_RL = cmd_msg->twist.angular.y;
  }

//  void flipperCallback(const geometry_msgs::Vector3::ConstPtr& cmd_msg)
//  {
//    boost::mutex::scoped_lock scoped_lock(lock_);
//    Target_FLP_FR = cmd_msg->x;
//    Target_FLP_FL = cmd_msg->x;
//    Target_FLP_RR = cmd_msg->y;
//    Target_FLP_RL = cmd_msg->y;
//  }
/*MEMO
...
*/
      ...
      gazebo_ros_->getParameter<std::string> (topicname_flpinfo_,
                                             "drrobot_jaguar_v6_flipper_info", topicname_flpinfo_);
...
//      ros::SubscribeOptions flpinfo =
//        ros::SubscribeOptions::create<geometry_msgs::Vector3>
//            (topicname_flpinfo_,
//            1, boost::bind(&MobileBasePlugin::flipperCallback, this, _1),
//            ros::VoidPtr(), &queue_);
//      flipper_info_subscriber_ = gazebo_ros_->node()->subscribe(flpinfo);
