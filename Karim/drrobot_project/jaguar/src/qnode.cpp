...
bool QNode::init() {
    ...
    stairs_distance_subscriber = n.subscribe<std_msgs::Int32>("stairs_distance", 1, boost::bind(&QNode::stairsApproachPhase, this, _1));
...
void QNode::flipCmdReceived(const geometry_msgs::TwistStamped::ConstPtr& cmd_msg)
{
    ...
    emit flipCmdUpdated(cmdFF,cmdFR);
...
void QNode::ButtonCmdReceived(const sensor_msgs::JoyConstPtr &joy)
{
    ...
    if(joy->buttons[11]){
        emit StairsAutoMotion();
        std::cout << "Stairs auto motion %d" << std::endl;
        stairsDistance = true;
    }

}

void QNode::stairsApproachPhase(const std_msgs::Int32::ConstPtr& distance)
{
    if(stairsDistance){
        std::cout << "Stairs Approach" << std::endl;
        if(distance->data > 90){
            ROS_INFO("Distance : %d", distance->data);
            emit StairsApproach(-100,100);
        }
        else {
            stairsDistance = false;
            ROS_INFO("Approach finished : %d", distance->data);
        }
    }
}
